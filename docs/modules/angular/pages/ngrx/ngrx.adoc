...
The components of this dialogs are subcomponents of the page they are used in. The subcomponents are placed in a subfolder called 'dialogs'. They do not have a septate section in the store because they are sharing the data of the page they are belonging to. The viewmodel can be passed via an input which is called 'vm'. The viewmodel is read from the store via a viewmodel selector in the effects or build with data from the action which is triggering the opening of the dialog.
